...
const ProductCard = ({brand, name, price, publicImageUrl, rating, link} : BestSellersExtendedType) => {
  return (
    <div className="relative flex flex-col border rounded-xl p-3 shadow-lg overflow-hidden dark:border-white/20 max-w-100">
      {link ? (
        <Link
          href={link}
        ...
        >
          <ProductImage pubilcId={publicImageUrl} alt={`${brand} ${name}`} quality="auto" />
        </Link>
      ) : (
        <div className="h-2/3">
          <ProductImage pubilcId={publicImageUrl} alt={`${brand} ${name}`} quality="auto" />
      ...
      <div className="px-3 flex-1">
        ...
        <div className="w-full flex justify-between items-center">
          <div className="flex flex-col justify-center items-start">
            ...
            <h3 className="py-1 font-general-sans">INR {price}</h3>
          </div>
        ...
        {link ? (
          <Link
            href={link}
            className="bg-foreground dark:bg-white/10 text-background dark:text-[#E5C3A6] border px-4 py-2 rounded-full cursor-pointer flex justify-center items-center font-general-sans my-2 text-sm hover:bg-foreground/80 hover:dark:bg-white/5 transition-color duration-500"
          >
            Buy Now
          </Link>
        ) : (
